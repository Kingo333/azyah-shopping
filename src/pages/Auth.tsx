...
const Auth = () => {
  ...
  const handleGoogleSignIn = async () => {
    ...
    setIsLoading(true);
    try {
      // Intent-based sign-in with correct redirectTo format
      const intent = activeTab === 'signin' ? 'signin' : 'signup';
      const role = selectedRole || 'shopper';
      const redirectTo = `${window.location.origin}/auth/callback?intent=${intent}&role=${role}`;
      const {
        error
      ...
      // Store role and intent for callback
      if (selectedRole) {
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
              ...
              <TabsContent value="signup" className="space-y-6 mt-4">
                ...
                <div className="space-y-3">
                  ...
                  <Button type="button" variant="premium" size="lg" className="w-full h-12 text-sm font-semibold" onClick={handleGoogleSignIn} disabled={isLoading || !selectedRole}>
                  ...
                  {!selectedRole && <p className="text-xs text-amber-600 bg-amber-50 dark:bg-amber-950/20 dark:text-amber-400 px-2 py-1.5 rounded text-center">
                ...
                <form onSubmit={handleSignUp} className="space-y-4">
                  ...
                  <div className="space-y-1">
                    ...
                    <Input id="signup-password" name="password" type="password" placeholder="Create password (8+ characters)" required minLength={8} value={signupForm.password} onChange={e => handleSignupFormChange("password", e.target.value)} className="h-10 glass-panel border-white/20 text-sm" />
