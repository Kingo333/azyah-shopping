...
const beautyTips = [{
  icon: "💄",
  tip: "Apply lipstick in thin layers for longer-lasting color"
}, {
  icon: "🌸",
  tip: "Always moisturize before applying foundation"
}, {
  icon: "✨",
  tip: "Use primer to make your makeup last all day"
}, {
  icon: "🎨",
  tip: "Blend eyeshadow in circular motions for smooth gradients"
}, {
  icon: "💫",
  tip: "Set your makeup with translucent powder"
}];
export default function BeautyConsultantPage() {
  ...
  const handleSendMessage = async () => {
    if (!inputMessage.trim() && !selectedImage && analysisMode === 'chat' || analysisMode === 'product_analysis' && !productImage && !skinImage && !inputMessage.trim()) {
      return;
    }
  ...
  return <div className="dashboard-bg">
      ...
      <main className="container mx-auto px-4 pt-4 max-w-4xl">
        ...
        {/* Tab Navigation */}
        <Card className="glass-panel mb-1 border-0">
          <CardContent className="p-1">
            <Tabs value={analysisMode} onValueChange={value => setAnalysisMode(value as 'chat' | 'product_analysis')} className="w-full">
              <TabsList className="grid w-full grid-cols-2 bg-transparent">
                <TabsTrigger value="chat" className="data-[state=active]:bg-primary data-[state=active]:text-white data-[state=active]:shadow-lg rounded-lg transition-all duration-300">
                  <Sparkles className="w-4 h-4 mr-2" />
                  Chat Mode
                </TabsTrigger>
                <TabsTrigger value="product_analysis" className="data-[state=active]:bg-primary data-[state=active]:text-white data-[state=active]:shadow-lg rounded-lg transition-all duration-300">
        ...
        <Card className="glass-premium mb-1 border-0 overflow-hidden">
          <CardContent className="p-0">
            <div className="h-[450px] overflow-y-auto scrollbar-hide p-2">
              <div className="space-y-2">
                ...
                animationDelay: `${index * 0.1}s`
                    ...
                    <div className={`flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center shadow-lg ${message.type === 'user' ? 'bg-gradient-to-br from-primary to-primary-glow' : 'bg-gradient-to-br from-accent/80 to-primary/60'}`}>
                    ...
                    <div className={`flex flex-col max-w-[80%] ${message.type === 'user' ? 'items-end' : 'items-start'}`}>
                      <div className={`relative p-4 rounded-2xl shadow-lg backdrop-blur-sm ${message.type === 'user' ? 'bg-primary/90 text-white rounded-tr-md ml-4' : 'bg-white/80 dark:bg-card/80 text-foreground rounded-tl-md mr-4 border border-white/20'}`}>
                        <div className="text-sm leading-relaxed">
                          {message.isVoice ? <div className="flex items-center gap-2">
                        ...
                        {message.image && <img src={message.image} alt="User uploaded content" className="mt-3 max-w-[250px] rounded-xl border border-white/30 shadow-md" />}
                      </div>
                      
                      {message.audioUrl && message.transcription && <div className="mt-3 max-w-[300px]">
                          <VoiceMessage audioUrl={message.audioUrl} transcription={message.transcription} />
                        </div>}
                ...
                {loading && <div className="flex items-start gap-4 animate-scale-in">
        ...
        {/* Image Preview Cards */}
        {(imagePreview || productImagePreview || skinImagePreview) && <div className="mb-1 space-y-1">
            ...
            {analysisMode === 'product_analysis' && (productImagePreview || skinImagePreview) && <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {productImagePreview && <Card className="glass-panel border-0 animate-scale-in">
                ...
                {skinImagePreview && <Card className="glass-panel border-0 animate-scale-in">
                    <CardContent className="p-4 flex items-center gap-3">
          ...
          <CardContent className="p-1">
            <div className="flex items-center gap-3">
                ...
                {analysisMode === 'chat' ? <TooltipProvider>
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <Button variant="outline" size="icon" onClick={() => fileInputRef.current?.click()} className="rounded-full border-2 hover:border-primary/50 hover:bg-primary/5 transition-all duration-300" disabled={loading}>
                    ...
                    <TooltipProvider>
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <Button variant="outline" size="icon" onClick={() => productFileInputRef.current?.click()} className={`rounded-full border-2 transition-all duration-300 ${!productImage ? 'border-blue-500/50 bg-blue-500/5 animate-pulse' : 'border-border hover:border-primary/50'}`} disabled={loading}>
                            <Package className="w-4 h-4 text-blue-500" />
                          </Button>
                        </TooltipTrigger>
                    ...
                    <TooltipProvider>
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <Button variant="outline" size="icon" onClick={() => skinFileInputRef.current?.click()} className={`rounded-full border-2 transition-all duration-300 ${!skinImage ? 'border-amber-500/50 bg-amber-500/5 animate-pulse' : 'border-border hover:border-primary/50'}`} disabled={loading}>
                            <User className="w-4 h-4 text-amber-500" />
                          </Button>
                        </TooltipTrigger>
              ...
              <div className="flex-1 relative">
                <Input value={inputMessage} onChange={e => setInputMessage(e.target.value)} onKeyPress={handleKeyPress} placeholder={analysisMode === 'product_analysis' ? !productImage || !skinImage ? "Upload both images to analyze compatibility..." : "Ask about product compatibility..." : "Ask about skincare, makeup, or beauty tips..."} className="h-12 pr-12 rounded-full border-2 bg-background/50 backdrop-blur-sm focus:border-primary/50 transition-all duration-300" disabled={loading} />
                <Button onClick={handleSendMessage} size="icon" className="absolute right-1 top-1 h-10 w-10 rounded-full bg-primary hover:bg-primary/90 shadow-lg" disabled={loading || !inputMessage.trim() && !selectedImage && analysisMode === 'chat' || analysisMode === 'product_analysis' && (!productImage || !skinImage) && !inputMessage.trim()}>
