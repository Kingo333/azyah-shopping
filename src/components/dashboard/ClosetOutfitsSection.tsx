  ...
  return (
    <section className="px-4 pt-1">
      ...
      <div className="grid grid-cols-2 gap-1.5">
        ...
        <div 
          onClick={() => handleNavigate('/dress-me/wardrobe')}
          className="bg-card rounded-lg p-1 border border-border/50 shadow-sm cursor-pointer hover:shadow-md transition-shadow flex flex-col max-h-[120px]"
        >
          {/* Grid of items with Create button */}
          <div className="grid grid-cols-2 gap-0.5 flex-1 max-h-[85px]">
            {/* Create Button - circular */}
            <div className="aspect-square flex items-center justify-center max-h-[40px]">
              <button 
                onClick={(e) => {
                  e.stopPropagation();
                  handleNavigate('/dress-me/wardrobe');
                }}
                className="w-full h-full max-w-[22px] max-h-[22px] rounded-full bg-secondary flex items-center justify-center hover:bg-secondary/80 transition-colors"
              >
            ...
            {displayClosetItems.map((item) => (
              <div 
                key={item.id} 
                className="aspect-square rounded-md overflow-hidden bg-secondary/30 max-h-[40px]"
              >
                <img 
            ...
            {displayClosetItems.length < 3 && 
              Array.from({ length: 3 - displayClosetItems.length }).map((_, i) => (
                <div key={`empty-${i}`} className="aspect-square rounded-md bg-secondary/20 max-h-[40px]" />
              ))
            }
          </div>
