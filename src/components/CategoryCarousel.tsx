        ...
        <Button variant="ghost" className="w-full h-full p-0 hover:bg-transparent" onClick={onToggle}>
          <div className={cn("w-full rounded-lg overflow-hidden", category.color)}>
            ...
            <div className="grid grid-cols-2 gap-1 p-4 pt-2">
              {products.length > 0 ? products.slice(0, 4).map((product, index) => <div key={product.id} className="aspect-square bg-white rounded-lg overflow-hidden shadow-sm">
                    <img src={getPrimaryImageUrl(product)} alt={product.title} className="w-full h-full object-cover" onError={e => {
            ...
            Array.from({
              length: 4
            }).map((_, index) => <div key={`default-${index}`} className="aspect-square bg-white rounded-lg overflow-hidden shadow-sm">
                    <img src={category.defaultImage} alt={`${category.name} category`} className="w-full h-full object-cover" />
                  </div>)}
              
              {/* Fill remaining slots with default images if needed */}
              {products.length > 0 && products.length < 4 && Array.from({
              length: 4 - products.length
            }).map((_, index) => <div key={`default-fill-${index}`} className="aspect-square bg-white rounded-lg overflow-hidden shadow-sm">
                    <img src={category.defaultImage} alt={`${category.name} category`} className="w-full h-full object-cover" />
                  </div>)}
            </div>
