...
const ExploreSearch: React.FC<ExploreSearchProps> = ({ 
    ...
    queryFn: async () => {
      ...
      if (activeTab === 'all' || activeTab === 'users') {
        const { data: users } = await supabase
          .from('users')
          .select('id, name, email, avatar_url, bio, country')
          .or(`name.ilike.%${debouncedQuery}%,email.ilike.%${debouncedQuery}%,bio.ilike.%${debouncedQuery}%`)
          .limit(10);
        ...
        users?.forEach(user => {
          results.push({
            ...
            subtitle: user.bio || user.country || 'Fashion enthusiast',
      ...
      if (activeTab === 'all' || activeTab === 'products') {
        ...
        products?.forEach(product => {
          results.push({
            ...
            }).format(product.price_cents / 100)} • ${product.brands?.name || 'Unbranded'}`,
      ...
      if (activeTab === 'all' || activeTab === 'brands') {
        ...
        brands?.forEach(brand => {
          results.push({
            ...
            id: brand.id,
            title: brand.name,
            subtitle: brand.bio || 'Fashion brand',
  ...
  const handleResultClick = (result: SearchResult) => {
    ...
    switch (result.type) {
      ...
      case 'product':
        ...
        break;
      case 'brand':
        navigate(`/brand/${result.data.slug}`);
        break;
      case 'style':
        navigate('/swipe', { 
          state: { 
  ...
  const filteredResults = searchResults?.filter(result => 
    activeTab === 'all' || result.type === (activeTab === 'users' ? 'user' : activeTab.slice(0, -1))
  ) || [];
