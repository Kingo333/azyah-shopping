...
export class EnhancedAxessoClient {
  ...
  private startRateLimitProcessor(): void {
    setInterval(() => {
      const maxRequestsPerInterval = Math.floor(this.config.maxRpm / 60);
      let processed = 0;
      
      while (this.rateLimitQueue.length > 0 && processed < maxRequestsPerInterval && this.activeRequests < this.config.maxConcurrency) {
  ...
  private async executeWithRateLimit<T>(operation: () => Promise<T>): Promise<T> {
    return new Promise((resolve, reject) => {
      ...
      this.rateLimitQueue.push(wrappedOperation);
    });
  }
  ...
  async bulkSearchAndHydrate(
    ...
    allProducts.push(...results.filter(Boolean) as AxessoProductDetails[]);
    ...
    return {
      ...
      metrics: {
        ...
        successRate: ((metrics.detailRequests - metrics.errors) / metrics.detailRequests) * 100
      }
    };
  }
  ...
  clearExpiredCache(): void {
    ...
    for (const [key, entry] of this.searchCache.entries()) {
      if (now - entry.timestamp >= this.config.searchCacheExpiry) {
        this.searchCache.delete(key);
      }
    }
    
    for (const [key, entry] of this.detailsCache.entries()) {
      if (now - entry.timestamp >= this.config.detailsCacheExpiry) {
