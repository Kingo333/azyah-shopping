/**
 * Types for the Azyah WebView Extractor Capacitor Plugin
 * 
 * This plugin provides real in-session product extraction using native WebView
 * with JavaScript injection capability (unlike @capacitor/browser which uses 
 * system browsers that cannot inject JS).
 */
 ...
 */
export interface ExtractedWebViewContext {
 ...
 */
export interface WebViewExtractionPluginResult {
  /** Whether extraction was successful */
  success: boolean;
  
  /** Extracted product context (if successful) */
  context?: ExtractedWebViewContext;
...
export interface AzyahWebViewExtractorPlugin {
  openAndExtract(options: OpenAndExtractOptions): Promise<WebViewExtractionPluginResult>;
}
