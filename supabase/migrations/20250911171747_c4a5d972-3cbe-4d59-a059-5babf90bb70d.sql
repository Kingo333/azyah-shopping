-- Improve the delete_user_completely function to handle users that exist only in public schema
...
DECLARE
  ...
  temp_count integer;
BEGIN
  -- First, try to find user in public.users table
  ...
  RAISE LOG 'Starting deletion process for user_id: % with email: %', target_user_id, target_email;
