...
AS $$
BEGIN
  -- Log access attempt for monitoring (only for anonymous users)
  IF auth.uid() IS NULL THEN
    -- Try to insert log, but don't fail if events table has issues
    BEGIN
...
AS $$
BEGIN
  -- Log access for monitoring (only for anonymous users)
  IF auth.uid() IS NULL THEN
    BEGIN
      INSERT INTO events (event_type, event_data, created_at)
      VALUES (
