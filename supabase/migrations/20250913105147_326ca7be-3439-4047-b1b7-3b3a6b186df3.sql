...
-- Also fix the search path issue in the existing functions by updating them
CREATE OR REPLACE FUNCTION public.get_brand_safe_fields(brand_id_param uuid)
RETURNS TABLE(id uuid, name text, slug text, logo_url text, bio text, website text, socials jsonb, shipping_regions text[], cover_image_url text, created_at timestamp with time zone, updated_at timestamp with time zone)
LANGUAGE plpgsql
SECURITY DEFINER
