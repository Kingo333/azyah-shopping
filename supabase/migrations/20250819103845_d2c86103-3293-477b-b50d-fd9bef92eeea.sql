...
-- Drop constraint that depends on the function
ALTER TABLE products DROP CONSTRAINT IF EXISTS products_category_subcategory_check;

-- Drop the existing function
DROP FUNCTION IF EXISTS public.validate_category_subcategory(category_type, subcategory_type) CASCADE;

-- Remove men and women from category_type enum by recreating it
CREATE TYPE category_type_new AS ENUM ('clothing', 'footwear', 'accessories', 'jewelry', 'beauty', 'bags', 'modestwear', 'kids', 'fragrance', 'home', 'giftcards');

-- Update products table to use new category_type
ALTER TABLE products ALTER COLUMN category_slug TYPE category_type_new USING category_slug::text::category_type_new;

-- Replace the old type
DROP TYPE category_type;
ALTER TYPE category_type_new RENAME TO category_type;

-- Update subcategory_type enum
CREATE TYPE subcategory_type_new AS ENUM (
...
ALTER TABLE products ALTER COLUMN subcategory_slug TYPE subcategory_type_new USING subcategory_slug::text::subcategory_type_new;

-- Replace the old type
DROP TYPE subcategory_type;
ALTER TYPE subcategory_type_new RENAME TO subcategory_type;

-- Recreate the validation function with new types
...
-- Set default gender for existing products
UPDATE products 
SET gender = CASE
