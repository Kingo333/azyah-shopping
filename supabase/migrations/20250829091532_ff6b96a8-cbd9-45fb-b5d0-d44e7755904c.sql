-- Fix security issues with users table and payment data access

-- 1. First, let's create a payments table if it doesn't exist (seems to be missing from schema)
...
-- 2. Drop existing overly permissive policies on users table
DROP POLICY IF EXISTS "Admins can view all users" ON public.users;
DROP POLICY IF EXISTS "Email access strictly limited to profile owner" ON public.users;

-- 3. Create more restrictive user access policies with mandatory audit logging
CREATE POLICY "Users can view their own basic profile data" 
ON public.users 
FOR SELECT 
USING (
  auth.uid() = id AND 
  (SELECT public.log_user_data_access('VIEW_OWN_PROFILE', 'users', auth.uid())) IS NOT NULL
);

-- 4. Create a secure admin access function with mandatory logging
...
-- 5. Create admin policy that requires using the secure function
CREATE POLICY "Admins can access user data through secure function only"
ON public.users
FOR SELECT
USING (false); -- This forces admins to use the secure function instead of direct access

-- 6. Create email access function with strict logging
...
-- 7. Create restrictive payment access policies
...
-- 8. Create secure payment access function for system operations
...
-- 9. Enhance subscriptions table security
...
-- 10. Create function to update payment encryption reminder
...
-- 11. Add trigger for payment data audit
...
-- 12. Create function to validate admin justification for data access
