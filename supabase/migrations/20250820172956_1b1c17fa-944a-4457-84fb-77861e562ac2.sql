...
-- 3. Fix security definer function with proper search_path
...
-- 4. Fix users table RLS policies - remove overlapping policies and secure role updates
...
WITH CHECK (
  auth.uid() = id AND 
  -- Prevent role changes by regular users - only allow if role hasn't changed or user is admin
...
-- 5. Fix events table RLS - ensure users can only see their own events
...
-- 6. Fix swipes table RLS - ensure users can only see their own swipes
