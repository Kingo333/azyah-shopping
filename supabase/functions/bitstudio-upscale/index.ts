...
serve(async (req) => {
  ...
  try {
    ...
    // Create upscale job in database
    const { data: jobData, error: jobError } = await supabase
      .from('ai_tryon_jobs')
      .insert({
        user_id: user.id,
        provider: 'bitstudio',
        provider_job_id: image_id,
        status: 'pending'
      })
      .select()
      .single();
    ...
    const response = await fetch(`${BITSTUDIO_API_BASE}/images/${image_id}/upscale`, {
      method: 'POST',
      headers: {
    ...
    return new Response(
      JSON.stringify({ 
        job_id: jobData.id,
        id: image_id, // Return original image ID for polling
        result: result 
      }),
    ...
    return new Response(
      JSON.stringify({ error: 'Upscale failed', details: error.message }),
      { headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 500 }
    );
  }
});
