...
serve(async (req) => {
  ...
  try {
    ...
    if (!image_id || !prompt) {
      return new Response(
        JSON.stringify({ error: 'Image ID and prompt are required' }),
    ...
    const response = await fetch(`${BITSTUDIO_API_BASE}/images/${image_id}/video`, {
      method: 'POST',
      headers: {
    ...
    return new Response(
      JSON.stringify({ 
        job_id: jobData.id,
        id: image_id, // Return image ID for polling
        result: result 
      }),
